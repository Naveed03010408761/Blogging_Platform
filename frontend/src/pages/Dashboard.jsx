  ...
  const handleClick = async (e) => {
    ...
    if (!loading) {
      ...
      try {
        ...
        console.log(`Like operation completed for post: ${postId}`);
      } catch (error) {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Debug: Check post IDs
        posts.forEach((post, index) => {
          console.log(
            `Post ${index}: ID = ${post._id}, type = ${typeof post._id}`
          );
        });
                ...
                {recentPosts.map((post) => (
                  <tr key={post._id} className="hover:bg-gray-50">
                    ...
                    <td className="border px-4 py-2 text-xs font-mono">
                      {post._id} {/* Debug: Show the post ID */}
                    </td>
                  </tr>
                ))}
              </tbody>
